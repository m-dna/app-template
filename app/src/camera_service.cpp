...
CameraTask::CameraTask(osal::Timer* timer, osal::Queue* queue,
                       hal::Camera* camera, coal::Communication* communication)
...
CameraTaskState CameraTask::get_state() const { return state; }
...
void CameraTask::run(int iterations) {
  int count = 0;
  while (iterations < 0 ||
         count < iterations && state != CameraTaskState::STOPPED) {
    step(state);
    count++;
    timer->delay_ms(100);
  }
...
void CameraTask::start() { state = CameraTaskState::RUNNING; }

void CameraTask::stop() { state = CameraTaskState::STOPPED; }
